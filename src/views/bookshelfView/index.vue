<template>
    <div class="bookshelf-page">
        ...
        <header class="header">
            ...
            <button class="upload-btn" @click="showUploadModal = true">上传文档</button>
        </header>

        <!-- 书架列表 -->
        <section class="bookshelf">
            <div v-for="(book, i) in books" :key="book.name + i" class="book-card" :class="{ open: currentBook === i }"
                @click="openBook(i)">
                <div class="cover-bg"></div>
                <div class="cover-content">
                    <h3 class="cover-title">{{ book.name }}</h3>
                    <p class="cover-uploader">Uploaded by {{ book.uploader }}</p>
                </div>
            </div>
        </section>
        ...
        <!-- 阅读器 -->
        <div v-if="currentBook !== null" class="reader-overlay" @click.self="closeBook">
        ...
        <div class="no-more" v-if="!hasMore">
            没有更多了
        </div>
    </div>
</template>
...
async function loadPage(p: number) {
    ...
    loading.value = true
    try {
        const res = await getAllTxtFiles(p, limit)
        const list = res.data || []
        // 如果返回数量 < limit，说明没有更多
        ...
        for (const item of list) {
            try {
                ...
                books.value.push({ name: item.name, content: text, uploader: item.uploader })
            } catch (e) {
                // 某本书加载失败可以忽略
            }
        }
